...
**Revision:** r1 (2026-07-05) — S0 text revised after cross-model review to close findings **F2–F9**;
F1/F4/F10 were already resolved in the first committed version. See
`verification/review-records/2026-07-05-s0-stage-2-scoping.md`.
...
This Stage-2 increment **partially addresses** that prerequisite — for **mechanically pumped
flow-boiling loops only.** Capillary / passive two-phase (heat-pipe, loop-heat-pipe) architectures
and a complete Starcloud/Suncatcher architecture judgment remain **deferred** (§7); Stage 2 does
not retire the two-phase prerequisite in general.
...
**explicitly deferred** and named below. As stated in §1, this branch **partially addresses** the
two-phase prerequisite (mechanically pumped flow-boiling loops only), not in general.
...
trade-space content; cannot be ranked; **does not advance the roadmap's two-phase prerequisite in
a rankable way.** **Rejected as the primary target** — but its value is preserved by making the
**first build milestone (S2) deliberately screening-level** before full coupling.
...
- **Harder:** every new correlation must carry a cited validity range and a microgravity caveat;
  cases exceeding the CHF margin, failing to fully condense before the pump, failing the pump-inlet
  feasibility gate, or sitting on an unstable branch of the internal characteristic must be
  **rejected or de-ranked**, not ranked.
...
**Onset-of-boiling / regime policy (F2).** Rank-eligible Stage-2 cases are restricted to a
**defined saturated flow-boiling regime.** The onset-of-nucleate-boiling (ONB) / subcooled-boiling
transition is handled by a **source-gated ONB criterion** (cited, with validity range); **absent a
sourced criterion, cases at or below ONB are sensitivity-only, not rank-eligible.** The `x → 0`
recovery test (§6 gate 1) is necessary but **not sufficient**, so the plan carries **separate**
tests for subcooled forced convection, the ONB transition, saturated flow boiling, and `x → 0`
pressure-drop recovery — a single endpoint check must not stand in for the transition.

**Dryout / critical-heat-flux gate (director margin, §9.5; F5).** A cited CHF correlation sets the
maximum permissible **local** heat flux. **`q''` in `q''/CHF` is the local modeled wall heat
flux** — not a section- or chip-average. If local wall flux cannot be obtained from **sourced**
geometry / heat-spreading assumptions, the case is **blocked or sensitivity-only**; an average flux
is **never silently substituted** (permitted only if explicitly named and categorized as such).
Cases are banded by `q'' / CHF`:
...
**Two-phase pressure drop (director choice, §9.4; F7).** Separated-flow model:
...
sensitivity**. The **exact Chisholm variant, its regime assumptions, and range of use are recorded
in the S1 registry** (§5, §8); any sub-choice that materially changes rank order is a **recorded
sensitivity, not an invisible default.** `ΔP_accel` is the momentum change across the density ratio
`ρ_f/ρ_g`; `ΔP_static ≈ 0` in microgravity (a **recorded modeling decision**, not a silent
assumption). **Limiting case:** as `x → 0`, `φ² → 1` and `ΔP_2φ` recovers the Stage-1 single-phase
`ΔP`.

**Condenser + pump-inlet feasibility (F3).** The condenser rejects `Q_cond = ṁ (h_in − h_out)`
(including latent heat) to the Phase A radiator law at `T_sat(P_cond)`. The pump inlet must be
**liquid with a subcooling margin** — incomplete condensation (`x > 0` at pump inlet) rejects the
case. In addition, pump-inlet feasibility requires a **source-gated NPSH / cavitation margin** for
the modeled pump class; a case that is liquid at the inlet but **fails the NPSH margin is
rejected.** If an NPSH criterion is not sourced, pump-inlet cavitation feasibility is **explicitly
not modeled (idealized pump-inlet boundary)**, and no claim stronger than **idealized steady-state
thermal-hydraulic screening** is made.
...
**Named energy boundary (F8).** Transport lines are modeled as **adiabatic (no external line heat
leak)** — a **named modeling boundary and limitation**, not an incidental parenthetical. Energy
closure `Q_chip + P_pump = Q_cond` holds **under this boundary**; it is **not** a general closure of
a full spacecraft thermal loop.
...
**Microgravity caveat (load-bearing, §9.2; F6).** Nearly all flow-boiling heat-transfer and
two-phase pressure-drop correlations are derived from 1-g terrestrial data. Per the director ruling,
these **cited 1-g correlations are used as the reference** with an **explicit
microgravity-applicability limitation carried in every reported result** (exact ranking-scope
wording in §7); microgravity-specific sources are **not required as hard blockers up front**, and
**no microgravity validation is claimed**.
...
1. **Two-phase flow-boiling heat acquisition** — vapor quality, boiling heat-transfer coefficient,
   the ONB/saturated-regime policy, and the CHF/dryout rejection bands on **local** wall flux (§3).
   *(introduced at S2)*
2. **Two-phase pressure drop** — Lockhart–Martinelli/Chisholm reference multiplier (variant recorded
   in the registry) + acceleration term; Friedel sensitivity; microgravity static-term decision.
   *(introduced at S3)*
3. **Coupled two-phase steady-state solution** — loop + condenser + radiator solved together;
   liquid-at-pump + NPSH feasibility constraint; named adiabatic-line energy boundary; static
   Ledinegg / non-uniqueness guard; energy closure with pump heat in the rejected load.
   *(introduced at S4 — MAJOR)*
...
- **Flow-boiling heat-transfer correlation** — cited, with mass-flux / quality / pressure validity
  range recorded.
- **Onset-of-nucleate-boiling / subcooled-boiling criterion (F2)** — cited, with validity range; if
  unsourced, cases at/below ONB are sensitivity-only, not rank-eligible.
- **Two-phase pressure-drop correlation (F7)** — **Lockhart–Martinelli/Chisholm (reference)** and
  **Friedel (sensitivity)**, each cited with validity range; the registry records the **exact
  Chisholm variant, regime assumptions, range of use, and parameter source**, and flags any
  rank-changing sub-choice as a sensitivity.
- **Critical-heat-flux correlation (F5)** — cited, with validity range; feeds the `q''/CHF ≤ 0.5`
  band on the **local** wall flux. Local flux requires sourced geometry / heat-spreading
  assumptions; if unavailable, the case is blocked or sensitivity-only.
- **Pump-inlet NPSH / cavitation criterion (F3)** — source-gated for the modeled pump class; if
  unsourced, recorded as **not modeled (idealized pump-inlet boundary)**.
...
director's constraints and to the closed cross-model findings.

1. **Limiting-case recovery + transition coverage (F2).** As `x → 0` (subcooled), the two-phase
   loop reproduces the Stage-1 single-phase pumped-loop outputs within tolerance (`φ² → 1`, boiling
   HTC → single-phase forced convection). Because the endpoint check is necessary-not-sufficient,
   **separate** tests cover subcooled forced convection, the ONB transition, and saturated flow
   boiling. *(unit + integration)*
2. **Energy conservation under the named adiabatic-line boundary (F8).** `Q_chip + P_pump = Q_cond`
   **under adiabatic transport lines**; the loop enthalpy balance closes across latent + sensible
   terms. Recorded as closure under that boundary, not a general physical closure. *(integration)*
...
5. **Explicit rejection / de-ranking of out-of-domain cases.** `q''/CHF ≥ 1` (dryout, on **local**
   wall flux), incomplete condensation at the pump inlet, **NPSH/cavitation-margin failure (F3)**,
   unstable (negative-slope) operating point, or regime/pressure/quality outside the correlation
   domain → **rejected**; `0.5 < q''/CHF < 1` → **sensitivity only, not ranked**. Reported in all
   cases, never silently ranked. *(integration + trade-study)*
...
**Ranking-scope limitation (F6, exact wording — carry in §8 S6 and every ranked output).**
*"Two-phase ranked cases are rank-eligible only within the adopted 1-g reference-correlation model.
Rankings are not microgravity-validated and may change under microgravity-specific correlations or
...
judgment**; any implication that Stage 2 retires the two-phase prerequisite beyond **mechanically
pumped flow-boiling loops**.
...
| S1 | Two-phase property/correlation **registry** — saturation backend, boiling HTC, **ONB criterion (F2)**, LM/Chisholm **variant + regime (F7)** + Friedel, CHF (local-flux basis, F5), **NPSH criterion (F3)** — each with provenance + validity ranges | intermediate (a + c; d for contested) |
| S2 | Two-phase **acquisition/evaporator** module — quality, boiling HTC, ONB/saturated-regime policy, **CHF/dryout bands on local flux** (screening-level first) | intermediate (b + c) |
| S3 | Two-phase **pressure drop** + **condenser** + liquid-at-pump + **NPSH** constraint | intermediate (b + c) |
| **S4** | **Coupled two-phase steady-state solver** — baseline recovery + transition tests, energy closure under adiabatic-line boundary, Ledinegg/uniqueness guard, convergence/failure | **MAJOR** — cross-model; **stop** |
...
| **S6** | **Two-phase trade-study extension** — updated/new Pareto fronts (single- vs two-phase); **every ranked output tagged with the §7 ranking-scope limitation (F6)** | **MAJOR** — cross-model; **stop** |
...
Recorded rulings; these supersede the draft's open questions and govern S1+. The r1 revision closes
cross-model findings F2–F9 **without reopening** any decision below.
...
2. **Microgravity policy.** Use **cited 1-g correlations as the reference** with **explicit
   microgravity-applicability limitations** (ranking-scope wording in §7). Microgravity-specific
   sources are **not** required as hard blockers up front; **no microgravity validation is claimed**.
...
4. **Two-phase pressure-drop family.** **Lockhart–Martinelli/Chisholm = reference**; **Friedel =
   named sensitivity** (exact variant recorded in the S1 registry, §5).
5. **CHF/dryout margin.** Ranked cases require **`q'' / CHF ≤ 0.5`** on the **local** wall flux
   (§3). Above that: **reject or mark sensitivity — do not rank**. Modeling margin, **not** flight
   certification.
...
- **Cross-model adversarial review complete;** findings dispositioned in the review record —
  **F1/F4/F10 already resolved in the first committed version; F2/F3/F5/F6/F7 closed by this r1 text
  revision; F8/F9 closed as named-boundary/wording cleanup.**
- **Returned for human director + cross-model review**; the review record at
  ...
  disposition and is **CLOSED** before any S1 code.
...
framing) skills; recorded here for the handoff's skills log. r1 revision closes cross-model findings
F2–F9. All standing rules (no-invention, oracle-freeze, claim discipline, cross-model ≠ level d,
"certificate" = internal numerical convergence) carried in unchanged.*
