...
The six figures below are generated by `scripts/plot_trade_study.py` **from the committed [`trade-study-points.csv`](trade-study-points.csv)** (and the engine's front definitions), so they cannot drift from the tested data. Points are coloured by case; **Pareto-front members are highlighted** (black edge). Gate-rejected and nonconverged points have no feasible metric coordinates and are noted (not plotted) in each caption.

### modeled_mass_vs_load

![modeled_mass_vs_load Pareto front](trade-study-figures/modeled_mass_vs_load.png)

### pump_power_vs_delta_T

![pump_power_vs_delta_T Pareto front](trade-study-figures/pump_power_vs_delta_T.png)

### radiator_area_vs_temp

![radiator_area_vs_temp Pareto front](trade-study-figures/radiator_area_vs_temp.png)

### junction_margin_vs_load

![junction_margin_vs_load Pareto front](trade-study-figures/junction_margin_vs_load.png)

### inventory_containment_mass_vs_pressure

![inventory_containment_mass_vs_pressure Pareto front](trade-study-figures/inventory_containment_mass_vs_pressure.png)

### modeled_mass_vs_parasitic_power

![modeled_mass_vs_parasitic_power Pareto front](trade-study-figures/modeled_mass_vs_parasitic_power.png)
